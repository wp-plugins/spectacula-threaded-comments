...
hideMany:commentingL10n.rpl_hide_2.replace("%name%",'<span class="poster-name"></span>').replace("%count%",'<span class="post-count">&nbsp;</span>'),scrollOff:parseInt(commentingL10n.scroll_off,10),moveForm:function(c,b){var d=a("#"+c+" > .comment-body .comment-meta").next().text(),f=a("#"+c+" > .comment-body cite.fn").text();addComment.replying&&addComment.cancelReply();addComment.replying=b;a("#comment-form #comment").before(a('<blockquote class="reply-quote"><cite>'+f+"</cite><p>"+(130<d.length?
d.substring(0,129)+"&hellip;":d)+"</p></blockquote>").hide()).prev(".reply-quote").slideDown();a("#comment-form input#comment_parent").attr({value:b});a("#"+c+" > .comment-body ").find(".comment-reply-link").hide();a("#comment-form #cancel-comment-reply-link").show();"function"==typeof a.scrollTo&&a.scrollTo("#respond",{offset:addComment.scrollOff,duration:500,axis:"y",onAfter:function(){a("#comment").focus()}});return!1},cancelReply:function(){0!==addComment.replying&&(addComment.replying=0,a("#comment-form .reply-quote").slideUp(500,
function(){a(this).remove()}),a("#comment-form input#comment_parent").attr({value:0}),a("#comment-form #cancel-comment-reply-link").hide(),a("#commentlist").find(".comment-reply-link").show());a("#comment-form .submit").removeAttr("disabled").removeClass("disabled");addComment.startInterval(!0)},commentDepth:function(c){return c?(c=a("li#comment-"+c).attr("class").match(/\bdepth-(\d+)\b/i),0<parseInt(c[1],10)?parseInt(c[1],10):1):0},error:function(c){if("string"!==typeof c)return!1;a("#comment-form .textarea-border").after(a('<div class="error">'+
c+"</div>").hide()).next(".error").slideDown("slow",function(){var c=a(this);void 0!==a(this)[0].style&&(a(this).css("FILTER",""),a(this)[0].style.filter="");setTimeout(function(){a(c).slideUp("slow",function(){a(this).remove()})},5E3)});return!0},deleteComment:function(c,b){if(!a("ul#commentlist li#comment-"+c+", #trackback-list li#comment-"+c).length||"unapprove"==b&&a("ul#commentlist li#comment-"+c+" > .comment-body:has( .moderation )"))return!0;a("li#comment-"+c+" > .comment-body").slideUp("slow",
function(){a(this).text("Deleted comment").slideDown().parent("li#comment-"+c).addClass("deleted")});return!0},newComment:function(c,b,d,f,e){if(a("ul#commentlist li#comment-"+b+", #trackback-list li#comment-"+b).length)return a("ul#commentlist li#comment-"+b).find(".moderation").slideUp("slow",function(){a(this).remove()}),a("li#comment-"+b).hasClass("deleted")&&a("li#comment-"+b+" > .comment-body").slideUp("slow",function(){a(this).html(a(c).find(".comment-body").html().trigger("newComment")).slideDown("slow")}).removeClass("deleted"),
!0;"comment"==e&&0<d?a("ul#commentlist li#comment-"+d).length&&(a("ul#commentlist li#comment-"+d+" > ul.children").length||a("ul#commentlist li#comment-"+d+" > div.comment-body").after('<ul class="children"></ul>'),a("ul#commentlist li#comment-"+d+" > ul.children").append(a(c).hide().addClass("rolledup").trigger("newComment")),d=a("li#comment-"+d).attr("class").match(/(?:\s|^)depth-(\d+)\s?/i),null!==d[1]&&d[1].match(/\d+/)&&(a("ul#commentlist li#comment-"+b).removeClass("depth-1 depth-2 depth-3 depth-4 depth-5 depth-6 depth-7 depth-8 depth-9 depth-10").addClass("depth-"+
